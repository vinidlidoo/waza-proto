...
With [`just`](https://github.com/casey/just) installed (`brew install just`), `just test` runs all four tiers and prints a pass/fail/duration summary; `just --list` shows the per-tier recipes (`test-unit`, `test-e2e`, `test-ios-unit`, `test-ios-ui`). The raw commands below are what each recipe runs.
