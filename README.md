...
- `cd viewer && npm test` — Vitest unit tests for the Vercel token-mint APIs (`viewer/api/viewer-token.js` and `viewer/api/publisher-token.js`). Covers envelope verification, JWT minting (grants + TTL), missing-env behavior, and identity collision-resistance. ~200 ms.
