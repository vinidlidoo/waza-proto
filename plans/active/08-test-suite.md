...
- **Reused the existing `waza-proto` LiveKit room with a freshly-minted test invite.** Plan called this out as the leading option; no new credentials needed, no risk of CI test patterns landing in a real demo viewer (test invite identity is `e2e-publisher`, viewer identity has the `viewer-` prefix the production token endpoint uses). Switch to a dedicated `waza-proto-local-test` room if isolation becomes a concern.
- **Publisher auto-spawned via Playwright `globalSetup`** (`lk room join --publish ../assets/testsrc.h264`), torn down via `globalTeardown` reading the PID from a small file. Avoids a "did you start the publisher?" manual step. The 3s sleep after spawn is load-bearing — without it the viewer connects before the publisher's track propagates through the SFU and the test races a TrackPublished event.
- **`channel: 'chrome'` (system Chrome), not Playwright's vendored Chromium.** Vendored Chromium ships without proprietary codec support, so H.264 (the codec the test publisher uses, the codec the iOS app uses on real hardware in `.hvc1`/H.264 flows) is silently rejected — the track is signaled, the viewer's WebRTC layer emits PLI requests, but `TrackSubscribed` never fires in the SDK. System Chrome has the codecs compiled in. The publisher's `received PLI` log line in stage-5 troubleshooting was the smoking gun: signaling worked, decode didn't.
- **`dotenv` loaded twice: once in `globalSetup`, once in `local-server.js`.** Playwright spawns the `webServer` command as its own subprocess and does not inherit `process.env` mutations the test runner made. So the local HTTP server has to reload `.env` itself to see `LIVEKIT_*` / `INVITE_SIGNING_SECRET`.
- **Dropped the `#status` text assertion in favor of `videoWidth > 0`.** Initial version asserted `status` text flipped to `"receiving video from e2e-publisher"`. Empirically, index.html's `RoomEvent.TrackUnsubscribed` handler fires intermittently during LiveKit JS's simulcast/backup-codec negotiation and flips the label back to `"waiting for video…"` even while the `<video>` element stays attached with frames flowing. The visible video element with non-zero dimensions is the real signal; text is just a UX artifact. The "subscriber requested backup codec but no track found" SFU warning is benign noise — the primary H.264 track is still subscribed.
- **Local server imports the Vercel handler directly via `import tokenHandler from '../api/token.js'`** rather than running `vercel dev`. Adapts Node http req/res to the Vercel-style req/res shape (`.query`, `.status()`, `.json()`) at the boundary. Keeps the test exercising the same code path the deployed function runs — drift in `viewer/api/token.js` is caught even though Vercel runtime quirks aren't.
- **No umbrella `make test` target.** Plan called it optional. Three commands (`cd viewer && npm test`, the iOS xcodebuild, `cd viewer && npm run test:e2e`) is small enough to live as documentation in the README. Revisit if/when CI lands.
