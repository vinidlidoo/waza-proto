...
- **`project.pbxproj` edit was a no-op.** `WazaProtoTests` is an Xcode-16 `PBXFileSystemSynchronizedRootGroup` with no membership exceptions, so the four new `.swift` files are auto-compiled into the test target just by living in the folder. The "add files to their targets" step in the file-layout delta required zero pbxproj changes — confirmed by all four new suites running.
- **Built/tested on iPhone 17 / iOS 26.5 sim (`D3BC437C…`) with `CODE_SIGN_IDENTITY="-"`.** No `Wearables.configure()` bootstrap fatal appeared — the #197 ad-hoc-signing fix holds. Full suite green; the in-process MDK wiring tests run ~1.1 s each (incl. the base class's 1 s settle).
- **Stage-4 pre-check reconfirmed in the harness, not just from the interface.** The two `GlassesGatewayMDKTests` pass: a denied connected mock surfaces through `GlassesGateway.refreshCameraPermission()` as `cameraPermission == .denied`, granted as `.granted`. The probe's finding holds inside `MockDeviceKitTestCase`.
- **The plan's suggested invite break (drop the base64url `+→-` swap) does NOT bite — used `exp = iat` instead.** `InviteTokenTests`' own decoder is encoding-tolerant (`base64URLDecode` maps `-→+`/`_→/` then pads, and `Data(base64Encoded:)` also accepts standard-base64 chars), so an envelope emitted with standard base64 still decodes and the signature still verifies — the swap is invisible to these semantic tests. The URL-safe encoding contract is instead covered end-to-end by the server's `viewer-token.test.js`. Verified break: dropping `+ Int(ttl)` from `exp` → `testPayloadWindowMatchesTTL` red.
- **Stage-4's done-criterion refined: the gate-fix revert (`!= .granted`) does NOT redden the MDK wiring tests; disabling the `refreshCameraPermission` assignment does.** The MDK tests only exercise `.denied`/`.granted` (no `nil`), and those two states behave identically under `== .denied` and `!= .granted`. So the gate predicate's edge is pinned by the pure `GlassesGateTests` `nil`-row (which the `!= .granted` break *does* redden), while the MDK test guards the *wiring* (`refreshCameraPermission` translation) — exactly the test-pyramid split this plan set out to make. Both confirmed red on their respective breaks.
- **All five deliberate breaks verified in one scoped `-only-testing` pass, then reverted.** smoother-underrun→`testUnderrunRepeatsLastFrame`; invite-ttl→`testPayloadWindowMatchesTTL`; gate-`nil`-row→`testRegisteredActiveNilPermissionYieldsNone`; wiring→both `GlassesGatewayMDKTests`. Exactly those five went red; every sibling test stayed green (no tautological tests, no over-broad breaks).
- **Optional XCUITest top layer consciously SKIPPED (not half-built).** `WazaProtoApp.swift` was left untouched — no `--mdk-camera-denied` launch-arg plumbing added. The in-process XCTest already tests the actual `refreshCameraPermission` translation where the bug lived (cheaper and more capable than the test server, which can't set permissions), and the granted/no-row case is already covered by the existing `ConnectFlowUITests.testPairingMockDeviceEnablesConnect`. Production change stays limited to the two pure refactors.
- **Gate enum relocated to a top-level `GlassesGateAction` in `GlassesGateway.swift`** (was a `private enum` in `ContentView`). Top-level rather than nested-in-`GlassesGateway` to keep the `nonisolated static` predicate free of the class's `@MainActor` isolation and let the truth table run off the main actor. `ContentView`'s `glassesGate`/`showGlassesGate` switch on it unchanged; the regression doc-comment moved with the logic.
- **Done-criterion #5 verified end-to-end against production (2026-05-29).** Built/installed/launched the app on the iPhone 17 sim from the CLI; tapped "Copy viewer link"; the minted URL decoded to a valid HS256 envelope with a 3 h window (`exp − iat = 10800`), and `GET https://waza-proto.vercel.app/api/viewer-token?invite=…` returned **200** with a LiveKit `token` + `url`. So the live server accepts an app-minted invite — closing the both-ends gap the invite refactor was about, not just the unit-level shape check.
