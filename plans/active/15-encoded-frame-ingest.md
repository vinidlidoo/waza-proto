...
- **Stage 2** — partially verified 2026-05-27. End-to-end pipeline works; matched-session three-run A/B at 720×1280 captured. Full report at [`plans/features/encoded-ingest-ab.md`](../features/encoded-ingest-ab.md). **Latency parity confirmed** (jb_per_frame within 15 ms band between paths). **Image quality wins from no-transcode-loss alone** (resolution matched between paths). **Freeze regression confirmed** (45–76 vs 28 events, 1.8–3.0s worst vs 0.7s) — smoother stall-masking is real and unreplicated in pass-through. **Three follow-ups before flipping `Config.glassesEncodedIngest = true` as default**: (a) SIGSEGV crash on accessory disconnect — blocking; (b) PTS-paced iPhone-side TCP writer to restore stall-masking — plan-sketched, expected to close most of the freeze gap; (c) optional control test with file-source through the relay to prove relay timing-neutrality.
