...
- **Freeze regression is PLI-deadlock, not BT-burstiness (2026-05-28 — supersedes prior framing).** Worked on this all afternoon as [plan 16](../completed/16-encoded-smoother.md), trying to fix freezes with an iPhone-side smoother. Three implementations all made it worse. Then accidentally observed: in unsmoothed mode, **refreshing the browser tab clears the freeze** even with the publisher unchanged. The freeze is the viewer's decoder hitting a missing reference and sending a Picture Loss Indication to the SFU; SFU forwards to lk-cli (the publisher); lk-cli can't generate a keyframe on demand (it just forwards bytes); browser eventually times out and stays frozen until refresh creates a new subscriber that gets the next IDR. **Re-encode wins the freeze column specifically because its in-app H.264 encoder responds to PLI by emitting an immediate IDR.** Encoded pass-through can't (no encoder in path). The 9× regression is architectural, not a smoothing gap. Mitigations sketched in [plan 16 §Possible follow-ons](../completed/16-encoded-smoother.md#possible-follow-ons-if-encoded-default-ever-revisits): force higher IDR cadence in DAT (if SDK allows), build a stateful relay that replays IDR on PLI, or wait for native LiveKit Swift HEVC ingest. None committed to.
...
- **Stage 2** — measured across two BT regimes 2026-05-27/28. End-to-end pipeline works in both encoded and re-encode modes. Lucky-regime three-run A/B at 720×1280 (evening of 2026-05-27) and stressed-regime two-run A/B at 504×896 (morning of 2026-05-28) both captured. Full report at [`plans/features/encoded-ingest-ab.md`](../features/encoded-ingest-ab.md). **Latency parity confirmed in both regimes** (jb_per_frame Δ ≤ 15 ms). **Image quality wins from no-transcode-loss alone** (resolution matched between paths). **Freeze regression root-caused as PLI-deadlock**, not BT-burstiness — see Decisions logged. **Updated gating before flipping `Config.glassesEncodedIngest = true` as default**: (a) SIGSEGV crash on accessory disconnect — blocking; (b) **freeze-recovery story for HEVC pass-through** — see [plan 16 §Possible follow-ons](../completed/16-encoded-smoother.md#possible-follow-ons-if-encoded-default-ever-revisits) for the three sketched paths; none committed to; (c) optional control test with file-source through the relay to prove relay timing-neutrality.
