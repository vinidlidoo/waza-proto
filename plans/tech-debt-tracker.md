...
- [ ] [Glasses stream: watchdog misses link-level disconnects](tech-debt/glasses-watchdog-eaaccessory-disconnect.md) — a silent BT link drop doesn't always promote to DAT's session state/error streams, so `onTerminated` never fires and the UI shows "Connected" while the glasses are detached. Fix is to watch `activeDeviceStream()` go nil, not `EAAccessory` (pre-existing, plan 13; path-independent).
